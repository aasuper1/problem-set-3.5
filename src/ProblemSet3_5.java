...
public class ProblemSet3_5 {
	...
	public void leapYears(int count) {
		...
		if (count == 1){
			System.out.print("The next leap year is 2020.");
		}
		int counter = 1;
		if (count == 2){
	...
	public void palindromicNumbers(int number) {
		...
		int temp = number;
		do {
			length = (tempNum > 0) ? (length + 1) : (length);
			tempNum = tempNum / 10;
		} while (tempNum > 0);
		
		for (int x = 0; x < length; x++){
			if (number/((int)Math.pow(10, length-1-x)) == temp%10){
				number = number % (int)Math.pow(10, length-1-x);
				temp = temp/10;
			} else {
				palindrome = false;
			}
		...
		if (palindrome){
			System.out.println(storedNum + " is a palindromic number.");
		} else {
			System.out.println(storedNum + " is not a palindromic number.");
		}
	}
	...
	public void fibonacci(int n) {
		...
		if (14 >= n && n >= 11){
			System.out.println("The " + n + "th Fibonacci number is " + f2 + ".");
		} else {
			switch (n%10){
			case 1:
				System.out.println("The " + n + "st Fibonacci number is " + f2 + ".");
				break;
			case 2:
				System.out.println("The " + n + "nd Fibonacci number is " + f2 + ".");
				break;
			case 3:
				System.out.println("The " + n + "rd Fibonacci number is " + f2 + ".");
				break;
			default:
				System.out.println("The " + n + "th Fibonacci number is " + f2 + ".");
				break;
